-- =============================================================================
-- 000115 — Timetable substitute override writes the legacy FK safely
--
...
--
-- 000113 made the resolution accept the canonical id, but the INSERT still
-- wrote that canonical uuid into `substitute_teacher_assignment_id`, whose
-- foreign key references the legacy `staff_assignments` table. Staging has
-- zero `staff_assignments` rows, so every substitute save failed with a
-- foreign-key violation after the scope checks passed.
--
-- This migration writes NULL into the legacy column whenever a canonical
-- teaching assignment was resolved (the canonical id is already carried by
-- `substitute_teaching_assignment_id` and `teaching_assignment_id`), and
-- keeps the legacy value for legacy identifiers. The scope validation, audit
-- event, outbox event, signature, and grants are unchanged. Forward-only from
-- 000114. Validated and applied by the central process.
-- =============================================================================
...
begin
  ...
  insert into public.timetable_overrides as existing (
    ...
    p_subject_id, p_room_id,
    case when v_teaching is not null then null else p_substitute_teacher_assignment_id end,
