...
export const identityModule: AdapterModule = {
  ...
  operations: [
    ...
    operation("staffInvites.create", z.object({ contact: z.string().email(), expiresAt: z.string().datetime(), displayName: z.string().min(2), roleCode: z.string().min(1), reason: z.string().min(3), academicYearIds: z.array(uuid).optional(), gradeSectionIds: z.array(uuid).optional(), subjectIds: z.array(uuid).optional() }), ({ supabase }, payload) => dispatchStaffInvitation(supabase, payload)),
