...
export function ResultsBatches({ batches: initial = null }: { batches?: EntryBatch[] | null }) {
  ...
  async function publish(ref: string) {
    ...
    setBusyRef(null);
    if (!result.ok) return reportErrors(result.errors);
    setLive(`Published as ${result.value.ref} (demo) — the portal report is live`);
    await refresh();
  }
  ...
  async function startCorrection(ref: string) {
    ...
    setCorrectionReason("");
    if (!result.ok) return reportErrors(result.errors);
    setLive(`${ref} correction v${result.value.version} started (demo)`);
    await refresh();
  }
  ...
  async function withdraw(ref: string) {
    ...
    setWithdrawalReason("");
    if (!result.ok) return reportErrors(result.errors);
    setLive(`${ref} withdrawn — the live portal publication was removed (demo)`);
    await refresh();
  }
  ...
  async function returnForCorrection(ref: string) {
    ...
    setReturnReason("");
    if (!result.ok) return reportErrors(result.errors);
    setLive(`${ref} returned to entry with a reason (demo)`);
    await refresh();
  }
...
function BatchRow({
  ...
  return (
    <>
      <tr>
        <td>
          <a className={styles.rowLink} href={`/staff/results/${batch.ref}`}>
