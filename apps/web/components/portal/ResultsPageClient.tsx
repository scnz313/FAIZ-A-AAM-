...
function ResultsBody({ initialPublications }: { initialPublications?: Publication[] | null }) {
  ...
  return <div className={styles.body}>
    ...
    <div className="tabs" role="group" aria-label="Select a term">{terms.map((item) => <a key={item.id} href={`/portal/results?term=${item.id}`} className={`${styles.tabLink}${item.id === term.id ? ` ${styles.tabLinkActive}` : ""}`} aria-current={item.id === term.id ? "true" : undefined}>{item.label}</a>)}</div>
    ...
    <section aria-labelledby="publications-heading"><p className="section-label" id="publications-heading">Released reports</p><ul className={styles.publicationList}>{publications.map((publication) => <li key={publication.ref} className={styles.publicationRow}><a className="link-arrow" href={`/portal/results/${publication.ref}`}>{publication.term} · released report →</a><span className={styles.publicationMeta}><span className="num">{publication.ref}</span><span> · {formatKolkata(publication.publishedAtIso, { format: "day" })}</span><span> · v{publication.version}</span></span></li>)}</ul></section>
    <p className={styles.demoNote}><span className="demo-badge">Demo data</span><span>{ACADEMICS_DEMO_NOTE} Supabase mode reads the released report manifest and active-child snapshot.</span></p>
  </div>;
}
