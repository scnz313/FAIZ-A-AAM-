...
This section supersedes every execution order below, including Phase 11 and C0–C5. Those sections remain historical architecture and backlog references. The user approved completing and accepting **V15 UI** first (V15 supersedes V14, 8 September 2026), then database integration by workflow, then whole-application acceptance, then providers. Only this file defines the active order; current evidence is recorded in PROJECT-STATUS.md.

1. **Recovery checkpoint (active).** Inspect the dirty tree, classify existing UI/backend/test/document changes, map actual routes to V15 screens, and run the current local tests. Preserve all changes. Do not commit unrelated work together or assign overlapping files to multiple writers. Exit: current failures and unverified areas are recorded and each route has a checklist row.
2. **V15 UI acceptance.** Finish one group at a time: tokens/primitives → public/Guardian/staff shells → public pages → identity → admission/job forms and status → Guardian pages → Principal workspaces → Administrator workspaces → state gallery/QA harness. Limit patches to presentation, layouts, UI interactions, and relevant regression tests. Record domain defects for step 3. For each group compare the actual rendered screen against V15, click its controls, check empty/error/loading states, keyboard/focus, and mobile/desktop behavior. Where V15's own responsive or accessibility behavior is weak, fix beyond the prototype (owner decision). Copy V15 layout only — never its illustrative data, counts, actor names, or client-only authorization. Exit: every retained route has visual and interaction evidence, with provider-dependent controls labelled accurately.
3. **Local database integration.** Freeze the accepted visual design. Complete identity/context → people/guardian/teaching records → admissions/enrollment → finance → results → timetable → careers → content/support → imports/exports/documents → settings/audit/notifications. Each slice includes UI → service → database → returned state → affected projections. Test authorization, validation, stale version, retries, and reload persistence. Use native scratch PostgreSQL and explicit provider fakes; never install or use Docker. Demo-browser plus SQL tests alone do not prove the Supabase Auth/Storage runtime. Record that environment gap rather than claiming it passes. Exit: every slice has persistence and propagation evidence at the stated boundary.
4. **Whole-application acceptance.** Run real button/form journeys with fictional records across Administrator, Principal, Guardian, admission applicant, and job applicant. Test identical amounts/marks/references across producer and consumer pages, revocation, failure recovery, accessibility, responsive behavior, and all local quality gates. Exit: a reproducible acceptance record and no unresolved core workflow failure.
5. **Provider and release gate (deferred).** Reconfirm staging ledger and configuration before any remote work; verify Auth, Storage, scanner, PDFs, Resend/outbox, restore rehearsal, and provider failures. Vercel/production follow staging acceptance. No remote mutation is part of steps 1–4.

Work rule: one implementation owner, one active slice, then inspect diff → focused tests → manual interaction → relevant regression checks → status update. Do not restart design exploration or broaden module scope. Existing forward migrations must not be rewritten or renumbered from historical instructions.
