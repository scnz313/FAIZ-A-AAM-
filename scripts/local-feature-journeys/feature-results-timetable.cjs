...
module.exports = {
  async run(browser, base) {
    ...
    try {
      await staffAs(page, base, "/principal/results", { identity: STAFF_IDS.rania });
    ...
    try {
      ...
      await staffAs(page, base, "/principal/timetables", { identity: STAFF_IDS.rania });
      ...
      check("portal applies the override on Tuesday", tueBody.includes("N. Lone") && /override/i.test(tueBody), tueBody.slice(-120));
      ...
      await page.goto(`${base}/portal/timetable`, { waitUntil: "networkidle" });
      await page.waitForTimeout(1500);
      await page.getByRole("tab", { name: "Exam date sheet" }).click();
